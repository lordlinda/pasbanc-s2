/**home features */
export const features = [
  {
<<<<<<< HEAD
    title: "Plan",
    content:
      "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Pellentesque pulvinar fringilla diam, eu fringilla nisi pretium porta. Nam cursus, sapien sit amet dignissim auctor, arcu lorem convallis dui, eu feugiat metus nulla sit amet dolor. Nulla ",
=======
    title: "plan",
    content: "Planning is cools",
>>>>>>> cfa0e94fb6fd0cf2aab43a009068b3b1e650da4b
    links: [
      {
        name: "link1",
        modalContent: "link1",
      },
      {
        name: "link2",
        modalContent: "link2",
      },
      {
        name: "link4",
        modalContent: "link4",
      },
      {
        name: "link4",
        modalContent: "link4",
      },
    ],
  },
  {
<<<<<<< HEAD
    title: "Save",
    content:
      "saving Lorem ipsum dolor sit amet, consectetur adipiscing elit. Pellentesque pulvinar fringilla diam, eu fringilla nisi pretium porta. Nam cursus, sapien sit amet dignissim auctor, arcu lorem convallis dui, eu feugiat metus nulla sit amet dolor. Nulla . ",
    links: [
      {
        name: "link3",
        modalContent: "link3",
      },
      {
        name: "link4",
        modalContent: "link4",
      },
      {
        name: "link4",
        modalContent: "link4",
      },
      {
        name: "link4",
        modalContent: "link4",
      },
    ],
  },
  {
    title: "Invest",
    content:
      "investing Lorem ipsum dolor sit amet, consectetur adipiscing elit. Pellentesque pulvinar fringilla diam, eu fringilla nisi pretium porta. Nam cursus, sapien sit amet dignissim auctor, arcu lorem Vestibulum maximus vestibulum lectus, non facilisis nunc ultricies non. ",
    links: [
      {
        name: "link3",
        modalContent: "link3",
      },
      {
        name: "link4",
        modalContent: "link4",
      },
      {
        name: "link4",
        modalContent: "link4",
      },
      {
        name: "link4",
        modalContent: "link4",
      },
    ],
  },
  {
    title: "Learn",
    content:
      "learning Lorem ipsum dolor sit amet, consectetur adipiscing elit. Pellentesque pulvinar fringilla diam, eu fringilla nisi pretium porta. Nam cursus, sapien sit amet dignissim auctor, arcu lorem convallis dui, eu feugiat metus nulla sit amet dolor. Nulla ",
    links: [
      {
        name: "link3",
        modalContent: "link3",
      },
      {
        name: "link4",
        modalContent: "link4",
      },
      {
        name: "link4",
        modalContent: "link4",
      },
      {
        name: "link4",
        modalContent: "link4",
      },
    ],
  },
  {
    title: "Supply",
    content:
      "saving Lorem ipsum dolor sit amet, consectetur adipiscing elit. Pellentesque pulvinar fringilla diam, eu fringilla nisi pretium porta. Nam cursus, sapien sit amet dignissim auctor, arcu lorem convallis dui, eu feugiat metus nulla sit amet dolor. Nulla consequat sollicitudin neque eget varius. Duis mattis nibh eget sem  ",
=======
    title: "save",
    content: "saving is fun",
>>>>>>> cfa0e94fb6fd0cf2aab43a009068b3b1e650da4b
    links: [
      {
        name: "link3",
        modalContent: "link3",
      },
      {
        name: "link4",
        modalContent: "link4",
      },
      {
        name: "link4",
        modalContent: "link4",
      },
      {
        name: "link4",
        modalContent: "link4",
      },
    ],
  },
]

/**
 * PLAN FEATURES
 */

export const planFeatures = [
  {
    title: "Plan 1",
    content: "plan 1 is fun",
    links: [],
  },
  {
    title: "Plan 2",
    content: "plan 2 is fun",
    links: [],
  },
  {
    title: "Plan 3",
    content: "plan 3 is fun",
    links: [],
  },
  {
    title: "Plan 4",
    content: "plan 4 is fun",
    links: [],
  },
]

export const learnFeatures = [
  {
    title: "learn 1",
    content: "learn is fun",
    links: [],
  },
  {
    title: "learn 2",
    content: "learn 2 is fun",
    links: [],
  },
  {
    title: "learn 3",
    content: "learn 3 is fun",
    links: [],
  },
  {
    title: "learn 4",
    content: "learn 4 is fun",
    links: [],
  },
]

export const investFeatures = [
  {
    title: "Invest 1",
    content: "invest 1 is fun",
    links: [],
  },
  {
    title: "invest 2",
    content: "invest 2 is fun",
    links: [],
  },
  {
    title: "Invest 3",
    content: "plan 3 is fun",
    links: [],
  },
  {
    title: "Invest 4",
    content: "invest 4 is fun",
    links: [],
  },
]

export const supplyFeatures = [
  {
    title: "supply 1",
    content: "supply 1 is fun",
    links: [],
  },
  {
    title: "supply 2",
    content: "plan 2 is fun",
    links: [],
  },
  {
    title: "supply 3",
    content: "plan 3 is fun",
    links: [],
  },
  {
    title: "supply 4",
    content: "plan 4 is fun",
    links: [],
  },
]

export const saveFeatures = [
  {
    title: "save 1",
    content: "save 1 is fun",
    links: [],
  },
  {
    title: "Save 2",
    content: "save 2 is fun",
    links: [],
  },
  {
    title: "Save 3",
    content: "svae 3 is fun",
    links: [],
  },
  {
    title: "Save 4",
    content: "save 4 is fun",
    links: [],
  },
]
